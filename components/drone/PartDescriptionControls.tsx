...
export default function PartDescriptionControls({
  ...
  onNext,
  onPrevious,
}: PartDescriptionControlsProps) {
  return (
    <div
      className={`flex w-full flex-col gap-3 lg:w-36 lg:flex-col lg:items-stretch ${
        isIntro
          ? 'items-start sm:flex-row sm:items-center sm:justify-start'
          : 'items-start sm:flex-row sm:items-center sm:justify-start lg:items-stretch'
      }`}
    >
      {!isIntro && (
          ...
          {Array.from({ length: totalSteps }).map((_, index) => (
            <span
              ...
              className={`h-1.5 rounded-full transition-all duration-300 ${
                index + 1 === step
                  ? 'flex-[2] bg-blue-400 shadow-[0_0_12px_rgba(96,165,250,0.55)]'
                  : 'flex-1 bg-white/20'
              }`}
            />
          ))}
