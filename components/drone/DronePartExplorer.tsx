...
export default function DronePartExplorer({ parts }: DronePartExplorerProps) {
  ...
  const isSpotlightActive = hasStartedTour && Boolean(selectedPart);
  ...
  const spotlightStyle = selectedPart
    ? {
        WebkitMaskImage: `radial-gradient(circle at ${selectedPart.right} ${selectedPart.down}, #000 0 ${selectedPart.spotlightRadius ?? '16%'}, transparent calc(${selectedPart.spotlightRadius ?? '16%'} + 8%))`,
        maskImage: `radial-gradient(circle at ${selectedPart.right} ${selectedPart.down}, #000 0 ${selectedPart.spotlightRadius ?? '16%'}, transparent calc(${selectedPart.spotlightRadius ?? '16%'} + 8%))`,
      }
    : undefined;
  ...
  useEffect(() => {
    ...
    if (!hasStartedTour || !selectedPart) {
    ...
    const partX = (Number.parseFloat(selectedPart.right) / 100) * scrollContainer.scrollWidth;
            ...
            {isSpotlightActive && (
              <>
                ...
                <div
                  ...
                  style={{
                    background: `radial-gradient(circle at ${selectedPart?.right} ${selectedPart?.down}, rgba(37,99,235,0.18) 0%, rgba(37,99,235,0.08) 18%, transparent 34%)`,
                  }}
                />
              </>
            )}
              ...
              parts.map((part) => (
                <button
                  ...
                  onClick={(event) => {
                    ...
                    selectPart(parts.findIndex((item) => item.id === part.id));
                  }}
                  aria-label={`Show ${part.title}`}
                >
                  <DroneHotspot active={hasStartedTour && selectedPart?.id === part.id} />
                </button>
              ))}
          </div>
        </div>
